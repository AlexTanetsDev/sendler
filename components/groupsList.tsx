...
export default async function GroupsList({ id }: Props) {
	...
	if (userGroups === undefined) throw new Error('Unable to fetch userGroups!');
				...
				{userGroups.map((userGroup: IGroupDatabase) => (
					<li key={userGroup.user_id}>
						<div className="flex py-3.5 text-xl font-montserrat font-normal">
							...
							<button className="row-table-btn mr-4">Редагувати</button>
							<button className="row-table-btn mr-4">Видалити</button>
							<button className="row-table-btn mr-4">Експорт</button>
						</div>
