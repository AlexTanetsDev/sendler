...
export default async function fetchUserHistoryDetails(
  ...
  const query = `
            SELECT cl.tel, cl.client_id, sh.alfa_name, sh.sending_permission, sh.text_sms, sh.sending_group_date, sg.group_name, u.user_name, COALESCE(ARRAY_AGG(rs.recipient_status), ARRAY[CAST('pending' AS status_type)]) AS recipient_status
            FROM send_groups sg
            ...
            GROUP BY cl.tel, cl.client_id, sh.sending_permission, sh.alfa_name, sh.text_sms, sh.sending_group_date, u.user_name, sg.group_name
        `;
