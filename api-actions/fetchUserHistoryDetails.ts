...
export default async function fetchUserHistoryDetails(
  ...
  const query = `
            SELECT cl.tel, cl.client_id, sh.text_sms, sh.sending_group_date, sg.group_name, u.user_name, COALESCE(ARRAY_AGG(rs.recipient_status), ARRAY[CAST('pending' AS status_type)]) AS recipient_status
            FROM send_groups sg
