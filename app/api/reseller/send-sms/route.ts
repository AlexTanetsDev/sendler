...
export async function POST(req: Request) {
  ...
  const historyId = await addSendingHistory(text, group_id);

  // const setSmsIdentificatorsRes = await addSmsIdentificators(historyId, clients, identificators);

  const statusRes = await addSmsStatus(historyId, clients, 'pending', identificators);
