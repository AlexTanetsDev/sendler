...
export const createSmsUrlStr = (clients: (IClientDatabase)[], text: string, userName: string): string => {
	...
	const str = clients
		.map((client) => {
			return `${distination}=${client.tel}&Data=${adaptedText.replace('%UserName%', userName).replace('+%Param1%', client.parameter_1 ? `${'+' + client.parameter_1}` : '').replace('+%Param2%', client.parameter_2 ? `${'+' + client.parameter_2}` : '')}&`;
		})
		.join("");
