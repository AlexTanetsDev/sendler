...
export default async function fetchUserHistoryDetails(
  ...
  const query = `
             SELECT cl.tel, sh.text_sms, sh.sending_group_date, sg.group_name, u.user_name, ARRAY_AGG(rs.recipient_status) AS recipient_status
            FROM send_groups sg
            ...
            GROUP BY sh.text_sms, sh.sending_group_date, u.user_name, sg.group_name
        WHERE history_id = $1
        RETURNING *
        `;
